...
public class Queue_Elements<Item> implements Iterable<Item>, Queue<Item> {
	...
	// to the elements that can be taken from the queue (head points to the oldest
	// element).
	// However, it is built in manner that requires a pointer to the newest element.
	private Element<Item> oldest;

	// This Element always points to the newest (tail-most) element in the
	// LinkedList referenced by oldest.
