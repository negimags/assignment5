...
public class BSTSimple<Key extends Comparable<Key>, Value> implements BSTdetail<Key, Value> {
	...
	/**
	 * Main to implement the function for calculating the root and the values
	 * @param args
	 */
	public static void main(String[] args) {

		//int mapsizvar=10;// counter to increase the value of tree by 100 for eacg it
		...
		for (int j = 0; j < 1000; j++) {
			...
			int mapsizvar=10;// counter to increase the value of tree by 100 for eacg it
			// generate a random  tree with 1000 keys 
			for (int i = 0; i < 10000+mapsizvar; i++) {
				map.put(rand.nextInt(200+mapsizvar), new Integer(1));
			}

			BSTSimple<Integer, Integer> tree = new BSTSimple<>(map);
			...
			for (int i = 0; i < 10000; i++) {
				if (rand.nextBoolean()) {
					...
					//inserting recordds
					tree.put(rand.nextInt(200+mapsizvar), new Integer(1));
				} else {
					//deleting records
					tree.delete(rand.nextInt(200+mapsizvar));
				}

			}
			int size = tree.size();// to calculate the results
			System.out.println("Size of Tree: "+size + "  Depth:" + tree.maxDepth(tree.root) + "  Square Root of N:" + Math.sqrt(size) + "  Log of N to base 2: " + Math.log(size) / Math.log(2));
			//mapsizvar+=100;;
